...
//function to write data to global buffer from host->device
 ...
 //function to the kernel and program for the demo application
    ...
    cl_int program1(Mat src,Mat output,cl_kernel *ke,cl_program program)
    {
        ...
        cl_int status;
                 ...
                 if(flag==0)
                 {
                      dimage1=create_rw_buffer(bytes,src.data,0,NULL);
                      flag=1;
                 }

                 size_t global[3] = {src.cols,src.rows,1};
                 ...
                 status= clSetKernelArg(kernel, 1, sizeof(cl_int),&src.rows);
                 CHECK_OPENCL_ERROR(status,"");

                 status= clSetKernelArg(kernel, 2, sizeof(cl_int),&src.cols);
                 CHECK_OPENCL_ERROR(status,"");


                 s=src.step;
                 status= clSetKernelArg(kernel, 3, sizeof(cl_int),&s);
                 CHECK_OPENCL_ERROR(status,"");

                 channels=(int)src.channels();


                 status = clSetKernelArg(kernel, 4, sizeof(cl_int),&channels);
                 CHECK_OPENCL_ERROR(status,"");
                 ...
                 //calling the kernel function for BGR2HSV
                ...
                //starting kernel for HSV2BGR
                 ...
                 local[2]=1;
                 //calling kernel
                status = clEnqueueNDRangeKernel(queue,kernel,2,NULL,global,local,1,event,&event[1]);
                 CHECK_OPENCL_ERROR(status,"");
                ...
                //kernel for BGR2GRAY
                 ...
                 global[2]=1;
                 local[2]=1;
                status = clEnqueueNDRangeKernel(queue,kernel,2,NULL,global,local,2,event,&event[2]);
                 CHECK_OPENCL_ERROR(status,"");
