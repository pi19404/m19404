...
// the ONLY way to make a copy of 'source' to this image
{
    ...
    SetSize(QSize(source.height,source.width));
...
void IntImage::Load(const CString& filename)
{
    IplImage* img=0;
    char x[1000];
   // system("ls /media/windows/WuJx/SkinColor/non-skin-images/1014159.jpg");
    //sprintf(x,"ls %s",filename.c_str());
   // system(x);
    img = cvLoadImage(filename.c_str(),0);
    if(img==0)
    {
        cerr << "image is null-" << filename.c_str() <<":"<< endl;
        perror("error is ");
        return;
    }

    //cerr << "completed loading the image " <<endl;


    SetSize(QSize(img->height,img->width));
...
void IntImage::SetSize(const QSize size)
...
// size.cx is the new height and size.cy is the new width
{
    ...
    if((size.width() == height) && (size.height() == width) && (buf != NULL) && (data != NULL) ) return;
...
{
    result.SetSize(QSize(int(height*ratio),int(width*ratio)));
    ratio = 1/ratio;
...
void IntImage::CalcSquareAndIntegral(IntImage& square, IntImage& image) const
{
    ...
    square.SetSize(QSize(height+1,width+1));
    image.SetSize(QSize(height+1,width+1));
