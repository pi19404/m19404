...
void calc_lums(const std::vector<std::vector<Rgb> > &raw) {
    //lum.assign(h, w);
    //cyx.assign(h, w);
    //lum.resize(h,vector<UC>(w));


    //cyx.resize(h,vector<UC>(w));

    lum.clear();
    lum.resize(h);
    for (int i = 0; i < h; ++i) lum[i].resize(w, 0);

    cyx.clear();
    cyx.resize(h);
    for (int i = 0; i < h; ++i) cyx[i].resize(w, 0);
    ...
    while (y1 < h) {
        ...
        while (x1 < w) {
            ...
            for (y = y1; y < std::min(y2, h); ++y)
                for (x = x1; x < std::min(x2, w); ++x)
                    cc += fabs(sr - lum[y][x]);
            //subtracting mean value from each pixel and computing the sum
