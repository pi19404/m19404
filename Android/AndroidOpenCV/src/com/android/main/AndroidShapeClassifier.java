...
import com.android.gesture.ShapeDataCapture;
 ...
 * </p>
 */
public class AndroidShapeClassifier extends GraphicsActivity {
    /** Used as a pulse to gradually fade the contents of the window. */
	public static String appname="AndroidShapeClassifier";
    private static final int MSG_FADE = 1;

    ShapeDataCapture panel;
    ...
    protected void onCreate(Bundle savedInstanceState) {
		...
		String appdir=path+"/"+AndroidShapeClassifier.appname;
		Settings set=new Settings(appdir,appdir);
		try
		{
		panel=new ShapeDataCapture(set);
		...
		{
			Log.e("EE","exception in creating data "+e.toString());
			e.printStackTrace();
		}
    }
    ...
    class PaintView extends View {
        ...
        private boolean onTouchOrHoverEvent(MotionEvent event, boolean isTouch) {
            ...
            if (action == MotionEvent.ACTION_DOWN || action == MotionEvent.ACTION_MOVE|| action == MotionEvent.ACTION_HOVER_MOVE) {
                ...
                for (int i = 0; i < N; i++) {
                    for (int j = 0; j < P; j++) {
                        paint(getPaintModeForTool(1, mode),
            ...
            if (action == MotionEvent.ACTION_DOWN)
            {
            			s="";
            			mView.clear();
            			panel.On(true,(int)event.getX(),(int)event.getY());
...
//            	mView.clear();
    			panel.On(false,(int)event.getX(),(int)event.getY());
    			Result r=panel.dollar2.result;
    			s="Template "+r.Name+":Score"+r.Score;
    			Paint paint1=new Paint();
             	 paint1.setColor(Color.RED);
             	 paint1.setTextSize(30);

             	 //mBitmap =Bitmap.createBitmap((int)panel.cimage.width(),(int)panel.cimage.height(), Config.ARGB_8888);
             	 //Bitmap.createBitmap(mBitmap,(int)0,(int)0,(int)panel.WIDTH,(int)panel.HEIGHT);
             	 //mBitmap.copyPixelsFromBuffer(panel.cimage.getByteBuffer());
             	 //mBitmap = mBitmap.copy(Config.RGB_565, true);
                 //mPaint.setColor(BACKGROUND_COLOR);
                 //mCanvas.drawPaint(mPaint);
            	 //mCanvas.setBitmap(mBitmap);
            	 mCanvas.drawText(s,40,30, paint1);
            ...
            if (mBitmap != null) {
                ...
                switch (mode) {
                    case Draw:
                        ...
                        mPaint.setAlpha(255);
                        drawOval(mCanvas, x, y, 16,16, 0, mPaint);
                        break;
                }
            }
            mFadeSteps = 0;
